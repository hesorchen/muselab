...
Skills are model-discovered capability packs. muselab exposes this directory
as a local Claude Agent SDK plugin, enables `skills="all"`, and lets the model
pick the relevant skill for each task.

skills 是模型可发现的能力包。muselab 把本目录作为本地 Claude Agent SDK
...
- 内置 skill 带 plugin 命名空间，可与同短名的用户全局 skill 共存
