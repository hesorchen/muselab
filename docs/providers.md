...
A single `Provider` entry in `backend/endpoints.py`'s `CATALOG`. See
[add-provider.md](add-provider.md) for the exact diff and the rationale
behind the per-session `CLAUDE_CONFIG_DIR` isolation.
