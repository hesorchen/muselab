...
`CLAUDE_CONFIG_DIR` points to the per-user persistent state directory
`${XDG_STATE_HOME:-~/.local/state}/muselab/vendor-cli`, and any Claude OAuth
credential found there is removed. This prevents third-party requests from
silently falling back to Anthropic while keeping their transcripts across
restarts and temporary-directory cleanup. Only `~/.claude/skills/` is mapped
into the isolated root so user Skills remain available; settings, hooks,
plugins, credentials, and other provider keys stay isolated. `MUSELAB_TOKEN`
is not passed to that subprocess.
