...
`grep` 只读取普通文件，跳过指向内部状态和回收站的别名，并避免在文件被替换为命名管道时阻塞。工作目录内的普通文件软链接仍可搜索。

文件名搜索与全文搜索使用设备、inode、纳秒修改时间及状态变更时间共同校验目录缓存；同步或解压保留修改时间时仍能发现文件变化，未变化目录则继续复用缓存，不重复调用 `scandir`。
