...
新配置的召回默认软时限为 2000 ms，已有配置保留所保存的值。检索或重排发生故障、超时时，尽力保留可用结果，必要时不注入记忆并继续回复。Dreamer、Verifier、索引和 Skill 学习在后台运行。
...
软时限从召回入口开始，覆盖近期证据、稠密与关键词检索、正文读取及可选重排。
配置最大值为 5 秒，facade 在 8 秒截止，早于 SDK 的 10 秒 Hook watchdog。
检索为正文读取预留少量时间，避免一个通道卡住后丢掉另一个通道已命中的结果。
重排失败时保留已读取的记忆，并标记为部分成功。

安全性能事件 `memory.recall_hook_start`、`memory.recall_finish`、
`memory.recall_hook_finish` 记录阶段状态、耗时、条数和是否注入，不包含查询或记忆正文。
