...
3. **询问你**的 archive 目录（Muse 唯一能读写的文件夹），默认 `~/muselab-archive`
...
检查项：uv 版本、claude CLI、MCP 运行时（uvx / npx）、`.env` 格式、
`MUSELAB_ROOT` 可写性、CLAUDE.md 存在、`uv sync` 状态、服务状态、HTTP 探测、
token 有效性、配置的 provider key。阻塞性失败时返回非零。
...
| Anthropic 模型 401 | `~/.claude` 缺失——执行一次 `claude login` |
