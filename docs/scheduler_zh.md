...
- **应用定时任务**：在工作台中配置并持久化保存，由 MuseLab 后端调度；服务重启后仍可继续执行计划。
- **SDK 原生 Cron**：由 Agent 在支持该工具的 CLI 运行时中创建，仅在所属运行时存续期间有效。运行时结束后不会保留，也不会自动转换为应用定时任务。

需要跨会话运行时或服务重启后继续执行的计划，应使用应用定时任务。下文介绍应用调度器的持久化、调度类型与配置。
