...
| Codex Gateway | `http://127.0.0.1:8766/anthropic` | ✅ 内置本地网关预设 |
...
对于 Codex/OpenAI 后端模型，内置的 **Codex Gateway** 预设假设用户自备一个运行在本机的 sidecar，并在 `http://127.0.0.1:8766/anthropic` 暴露 Anthropic Messages 兼容端点。muselab 不读取 Codex OAuth 文件，也不直接调用 OpenAI 原生接口。见 [codex-gateway_zh.md](codex-gateway_zh.md)。
