...
1. **工作目录**（`MUSELAB_ROOT` 加其他已登记目录）—— 你自己的文件，
2. **仓库** —— 配置与会话元数据，
...
| 每个额外登记的工作目录 | 该目录的文件与 `.muselab-dustbin/` | 恢复该目录及可恢复回收站所必需 |
| `$MUSELAB_ROOT/.muselab/workspaces.json` | 已登记工作目录的路径和名称 | 恢复选择器；迁移后对应路径仍须存在 |
...
2. 停掉服务。
3. 恢复 `$MUSELAB_ROOT/`（含 `.muselab/scheduler.json` 与 `.muselab/workspaces.json`）、需要保留的其他工作目录、仓库的 `.env` / `sessions/` / `mcp.json` / `provider_overrides.json`，以及 `~/.claude/`。
4. 确认恢复后的 `.env` 里 `MUSELAB_ROOT` 指向默认工作目录的新位置，并在选择器里更新失效路径。
5. 启动服务，依次切换已恢复目录并验证一段对话、一份文件预览、定时任务和历史。
