...
查询参数可能进入浏览器历史与代理日志。muselab 会抑制原始访问日志；慢请求和失败请求改为隐私安全的结构化摘要，不记录查询值，并规范化动态标识。反向代理有自己的日志，仍必须单独配置脱敏。公网部署必须使用 HTTPS。
...
`CLAUDE_CONFIG_DIR` 指向持久且按用户隔离的状态目录
`${XDG_STATE_HOME:-~/.local/state}/muselab/vendor-cli`，并清除可能出现的
Claude OAuth 凭据。这样既避免第三方请求静默回退到 Anthropic，也不会因重启或
清理临时目录丢失第三方会话。只有 `~/.claude/skills/` 会映射进隔离目录，使用户
Skills 保持可用；settings、hooks、plugins、凭据和其他 provider 的 key 仍然隔离，
`MUSELAB_TOKEN` 也不会传入该子进程。
