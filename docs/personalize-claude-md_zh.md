...
工作区不只是文件浏览的位置，也是 Agent 执行任务的项目环境。项目目录中的指令、记忆文件、文档和 Skills 可以保存项目约定、已有结论和操作流程，供后续任务继续使用。

指令与 Skills 按 SDK 规则发现，普通文档和记忆笔记由 Agent 按需读取；登记工作区不代表自动加载全部文件。项目内的记忆文件与 MuseLab 的[长期记忆](memory_zh.md)是不同机制，后者需要单独配置，也不按工作区划分独立记忆池。
...
工作区说明属于目录，不属于 muselab 实例。代码仓库、研究资料和运营数据可以分别登记为工作区，并各自维护不同的 `CLAUDE.md`。切换工作区时，文件树、预览与会话视图随之切换，新会话使用所选目录；已有会话和终端保留各自的工作目录。
