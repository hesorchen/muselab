...
def test_undo_toast_restores_closed_tab(page: Page, backend_url, auth_token):
    """Closing a tab should surface an undo toast; clicking it must restore
    the tab. Guards the closeChatTab dupe regression — when both
    definitions coexisted, only the second ran and no toast appeared."""
    _login(page, backend_url, auth_token)
    page.locator(SEL_TAB_NEW).click()
    expect(page.locator(SEL_TAB)).to_have_count(2)
    before = 2

    target_title = page.locator(f"{SEL_TAB} {SEL_TAB_NAME}").first.inner_text()
    page.locator(f"{SEL_TAB} {SEL_TAB_CLOSE}").first.click()
    expect(page.locator(SEL_TAB)).to_have_count(before - 1)

    undo = page.locator(SEL_TOAST_ACTION)
    expect(undo).to_be_visible(timeout=3000)
    undo.click()
    expect(page.locator(SEL_TAB)).to_have_count(before)
    # The restored tab should carry the same title.
    titles = page.locator(f"{SEL_TAB} {SEL_TAB_NAME}").all_inner_texts()
