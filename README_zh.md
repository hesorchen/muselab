...
- 🗂 **VS Code 风格多 tab 聊天**：会话间随意切换不丢失进行中的 stream；双击 tab 名 inline 重命名；右键（或手机端 ⋮）打开完整菜单
- 📱 **添加到主屏 PWA**——一份代码同时服务 iOS / Android / desktop；无需 App Store，无需 $99/年 Apple 开发者证书；支持 standalone 全屏模式 + 状态栏跟随主题色
- 🚀 三个 OS 各一条 install 命令，或从 GHCR 直接 `docker run`
- ⚡ ~8 k 行代码 · 152 项测试 · 可在 1 GB 内存 VPS 运行

<!-- HERO_GIF_PLACEHOLDER — 录制 20~30 秒屏幕演示（chat + 文件预览 + 模型切换），导出 gif / webp ≤2 MB，放到 docs/assets/，把本注释替换为：![](docs/assets/hero.gif) -->
...
- [手机端（PWA）](#手机端pwa) — 添加到主屏、standalone
...
muselab 自带 Web App Manifest + apple-touch-icon。部署到你自己的服务器后，可以将其添加到手机主屏，启动后体验接近原生 app：

- **一份代码**同时服务 iOS / Android / desktop——无需打包 `.ipa` / `.apk`，无需 App Store 审核
- **Standalone 模式**：无浏览器地址栏 / 标签栏，全屏 app shell
- **主题色感知**：iOS 状态栏跟随 light / dark mode
- **触屏适配**：输入框 ≥ 16 px（避免 iOS 自动放大）、禁用下拉刷新、键盘弹起时聊天自动跟随滚动

iPhone 上 Chrome 添加方法：⋮ 菜单 → **分享** → **添加到主屏幕** → 添加。Android Chrome 地址栏会主动提示「安装」。

> 此处与 self-host 立场一致：你的手机直连你自己的服务器，链路中没有 Apple / Google 签名的二进制，也没有第三方分发渠道。**自托管的初衷不被安装路径破坏。**

**尚未实现**（已在 roadmap）：Web Push 推送通知（长时任务完成提醒）、Service Worker 离线 UI 缓存。进度见 [TODO.md](TODO.md)。
