...
${HOST} {
  @client_error path /api/log/client-error
  request_body @client_error {
    # The application accepts at most 8 KiB. Keep a second, slightly looser
    # proxy boundary so oversized unauthenticated telemetry never reaches
    # the ASGI server while leaving room for transport framing differences.
    max_size 16KB
  }
