...
"@
  # UTF-8 WITHOUT BOM. `Set-Content -Encoding utf8` on PowerShell 5.1
  # (default on Windows 10/11) writes WITH BOM, and python-dotenv does NOT
  # strip it — so the first key becomes "﻿MUSELAB_TOKEN" and the
  # backend rejects every login with 401. Use the same WriteAllText pattern
  # we use for CLAUDE.md below.
